...
PlaylistModule.prototype.handleQueue = function (user, data) {
    ...
    /**
     * Subtitles files are only allowed for raw files currently
     */
    if (typeof data.subtitles !== "string" || data.type !== "fi") {
    ...
    if (data.type === "fi") {
        if (data.id.length > 255) {
            return user.socket.emit("queueFail", {
                msg: "Video URL is too long",
                link: link
            });
        }

        if (!util.isValidURL(data.id)) {
            return user.socket.emit("queueFail", {
        ...
        if (data.subtitles) {
            if (data.subtitles.length > 255) {
                return user.socket.emit("queueFail", {
                    msg: "Subtitles file URL is too long",
                    link: link
                });
            }

            if (!util.isValidURL(data.subtitles)) {
                return user.socket.emit("queueFail", {
                    msg: "Invalid subtitles URL",
                    link: link
                });
            }
        }
...
PlaylistModule.prototype._addItem = function (media, data, user, cb) {
    ...
    if (data.subtitles && media.type === "fi") {
        console.log(data.subtitles);
        media.meta.subtitles = data.subtitles;
    }
